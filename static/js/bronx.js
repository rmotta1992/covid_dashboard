...
d3.csv("../data/boroughs-case-hosp-death.csv").then(function(boroughData) {
  ...
  boroughData.forEach(function(d) {
    ...
    d.BX_CASE_COUNT = +d.BX_CASE_COUNT;
});
  ...
  var yScale = d3.scaleLinear()
    .range([chartHeight, 0])
    .domain([0, d3.max(boroughData, d => d.BX_CASE_COUNT)]);
  ...
  var drawLine = d3
    ...
    .y(d => yScale(d.BX_CASE_COUNT));

  chartGroup5.append("path")
  ...
  chartGroup5.append("g")
    ...
    .attr("cy", d => yScale(d.BX_CASE_COUNT))
    .attr("r", "10")
    .attr("fill", "gold")
    .attr("stroke-width", "1")
    .attr("stroke", "white")
  ...
  var toolTip = d3.tip()
    ...
    .offset([20, 90])
    .html(function(d) {
      return (`<strong>Confirmed Cases:</strong> ${d.BX_CASE_COUNT}<hr>Date: ${dateFormatter(d.DATE_OF_INTEREST)}`);
      
    });
  
  //Create the tooltip in chartGroup.
  chartGroup5.call(toolTip);
